...
if ($userrank["ADMIN_MANAGE_FORUMS"] !== "on") {
  ...
  if (isset($_POST['add_link'])) {
    ...
    if (!empty($_POST['name']) and !empty($_POST['type']) and !empty($_POST['link'])) {

      $insert = $bdd->prepare("INSERT INTO `" . $_Config_['Database']['table_prefix'] . "_footer`( `NAME`, `TYPE`, `LINK`, `ICON`) VALUES (?,?,?,?)");
      $insert->execute(array(htmlspecialchars($_POST['name']), htmlspecialchars($_POST['type']), htmlspecialchars($_POST['link']), htmlspecialchars($_POST['icon'])));

      $badgeerror = '<div class="alert alert-success"><strong><i class="fas fa-check-circle"></i></strong> This link has been created.</div><meta http-equiv="refresh" content="1;URL=?page=footerlinks">';
    } else {
      $badgeerror = '<div class="alert alert-danger"><strong><i class="fas fa-exclamation-circle"></i></strong> Please enter all fields.</div>';
    }
  }

  $lfb = $bdd->query("SELECT * FROM " . $_Config_['Database']['table_prefix'] . "_footer");

  if (isset($_GET['action']) and $_GET['action'] == "delete_link") {

    if (isset($_GET['linkid']) and !empty($_GET['linkid'])) {

      $dban = $bdd->prepare("DELETE FROM `" . $_Config_['Database']['table_prefix'] . "_footer` WHERE id = ?");
      $dban->execute(array(htmlspecialchars($_GET['linkid'])));
      echo '<meta http-equiv="refresh" content="0;URL=?page=footerlinks">';
    }
  }
}
        ...
        <tbody>
          <?php while ($s = $lfb->fetch()) { ?>
            <tr>
              ...
              <th scope="row"><?php echo $s['TYPE']; ?></th>
              <td><a href="?page=footerlinks&action=delete_link&linkid=<?php echo $s['id']; ?>"><i class="fas fa-ban text-danger"></i></a></td>
            </tr>
          <?php } ?>
        </tbody>
      </table>
    ...
    <div class="card-body">
      ...
      <form method="POST">
        ...
        <select class="form-control" name="type">
          <option>Pages</option>
          <option value="Links">Usefull link</option>
